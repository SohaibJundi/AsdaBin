...
namespace AsdaBin
{
  public partial class form_Main : Form
  {
    ...
    private void form_Main_Load(object sender, EventArgs e)
    {
      ...
      int[] versionValues = Enum.GetValues(typeof(EncryptionHelper.Version)).Cast<int>().ToArray();
      string[] versionNames = Enum.GetNames(typeof(EncryptionHelper.Version));
    ...
    private void btn_Detect_Version_Click(object sender, EventArgs e)
    {
      ...
      {
        int version = EncryptionHelper.detectVersion(File.ReadAllBytes(txt_path.Text));
    ...
    {
      process((file) => { return EncryptionHelper.decryptBin(file, (EncryptionHelper.Version)cmbx_Version_From.SelectedIndex); });
    }

    private void btn_Encrypt_Click(object sender, EventArgs e)
    {
      process((file) => { return EncryptionHelper.encryptBin(file, (EncryptionHelper.Version)cmbx_Version_To.SelectedIndex); });
    }

    private void btn_Convert_Click(object sender, EventArgs e)
    {
      process((file) => { return EncryptionHelper.encryptBin(EncryptionHelper.decryptBin(file, (EncryptionHelper.Version)cmbx_Version_From.SelectedIndex), (EncryptionHelper.Version)cmbx_Version_To.SelectedIndex); });
    }
